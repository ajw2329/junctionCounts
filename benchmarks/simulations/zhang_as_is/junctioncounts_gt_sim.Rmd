...
dpsi_gt_corr_inner_dpsi_0 <- dpsi_gt_measured %>% 
  ...
  ggscatter(x = "gt_dpsi", 
    ...
    ggtitle(expr("Correlation of junctionCounts to simulated data\nwith known ground truth "*Psi),
            subtitle = expr("Given junctionCounts inner |"*Delta*Psi*"| > 0")) + 
...
dpsi_gt_corr_inner_dpsi_0_event_types <- dpsi_gt_measured %>% 
  filter(inner_dpsi != 0) %>%
  ggplot(aes(x = gt_dpsi,
             y = mid_dpsi)) +
...
dpsi_gt_corr_event_types_r2 <- dpsi_gt_measured %>% 
  group_by(event_type) %>%
  summarize(R2 = round(cor(mid_dpsi, gt_dpsi, use = "complete.obs")^2, 3)) %>%
  rename(`Event Type` = event_type) %>%
  arrange(-R2) %>%
