...
fp_rate <- apply(thresholds_full, 1, function(x, df) df %>% 
        ...
        summarize(fp_rate = mean(fp)) %>% pull(fp_rate), df = fp_tp)
...
thresholds_fp_rate %>% filter(inner_dpsi_thresh > 0) %>% select(-dpsi_span_thresh) %>% group_by(inner_dpsi_thresh, min_jc_comparison_thresh) %>% summarize(mean_fp_rate = mean(fp_rate, na.rm = TRUE)) %>% ungroup() %>% ggplot(aes(x = min_jc_comparison_thresh, y = inner_dpsi_thresh, fill = mean_fp_rate, col = mean_fp_rate <= 0.05)) + geom_tile() + scale_fill_viridis_c()
```
