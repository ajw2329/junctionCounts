...
exp1_fpkm_tpm <- cbind(transcript_id,
                       sample_frac(exp1_fpkm_tpm[,c(2,3,4,8,9,10)]),
                       sample_frac(exp1_fpkm_tpm[,c(5,6,7,11,12,13)]))


# randomize order ot transcript names to proliferate changing splicing events

exp1_fpkm_tpm %>%
	write.table(file = "/public/groups/sanfordlab/people/anjowall/projects/rtpcr_validated/zhang_2014/simulations/cond_sep_shuffled_tx_ids/fpkm_tpm_table.tsv",
		sep = "\t",
		row.names = FALSE,
		col.names = TRUE,
		quote = FALSE)
...
simulate_experiment_empirical(fpkmMat = exp1_fpkm_mat, 
							  ...
							  fasta = exp1_transcript_fasta,
   							  ...
   							  seed=1247,
   							  outdir="/public/groups/sanfordlab/people/anjowall/projects/rtpcr_validated/zhang_2014/simulations/cond_sep_shuffled_tx_ids/fasta/",
   							  bias = "rnaf",
