...
def bootstrap_junction_counts(junction_only_count_dict, standard_event_dict, eij_only_count_dict, junction_indexed_event_dict, eij_indexed_event_dict, n_reads, forward_read, h5dset):
	...
	for n in range(0,int(n_reads)):
		...
		if len(index_collection) == 1000000 or n == n_reads - 1:
			...
			index_dict = Counter(index_collection)
...
def import_ioe(event_ioe):
	...
	for line in ioe:
		...
		total_transcripts = set(entry[4].split(","))
		excluded_form_transcripts = total_transcripts - included_form_transcripts

		event_transcript_dict[event] = [included_form_transcripts, excluded_form_transcripts]
	...
	return gene_event_dict, event_transcript_dict


def get_event_junction_percentiles(standard_event_dict, event_transcript_dict, junction_percentile_dict, transcript_dict):
	...
	for chrom in standard_event_dict:
		...
		for strand in standard_event_dict[chrom]:
			...
			for event in standard_event_dict[chrom][strand]:
				...
				standard_event_dict[chrom][strand][event]["junction_percentiles"] = {}

				for i, form in enumerate(["included", "excluded"]):

					for junction in standard_event_dict[chrom][strand][event][form + "_junction_counts"]:

						junction_is_ei = len(junction.split("_")) == 3

						# eij percentile must be calculated here

						percentiles = []

						for transcript in event_transcript_dict[event][i]:

							if junction in junction_percentile_dict[transcript]:

								percentiles.append(junction_percentile_dict[transcript][junction])

							elif junction_is_ei:

								position = int(junction.split("_")[1])

								position_contained_res = splice_lib.position_contained(transcript_dict[transcript]["exons"], position)

								if position_contained_res[0]:

									transcript_length = splice_lib.calc_length_exon_list(transcript_dict[transcript]["exons"])

									left_exons = transcript_dict[transcript]["exons"][0:transcript_dict[transcript]["exons"].index(position_contained_res[1])]
									right_exons = transcript_dict[transcript]["exons"][transcript_dict[transcript]["exons"].index(position_contained_res[1]) + 1:]

									left_dist = splice_lib.calc_length_exon_list(left_exons) + position_contained_res[1][1] - position_contained_res[1][0] + 1
									right_dist = splice_lib.calc_length_exon_list(right_exons) + position_contained_res[1][1] - position_contained_res[1][0] + 1

									if strand == "+":

										percentiles.append(float(left_dist)/transcript_length)

									else:

										percentiles.append(float(right_dist)/transcript_length)


						try:
							percentile = int(round(sum(percentiles)/len(percentiles)))
						except ZeroDivisionError:
							print junction
							print index
							print standard_event_dict[chrom][strand][event]
							print event_transcript_dict[event][index]
							for transcript in event_transcript_dict[event][index]:
								print junction_percentile_dict[transcript]
							sys.exit()

						standard_event_dict[chrom][strand][event]["junction_percentiles"][junction] = percentile		


def end_correct_counts(standard_event_dict, percentile_coverage):

	percentile_coverage_list = []

	perc_cov = open(percentile_coverage, "r")

	perc_cov.readline()

	for line in perc_cov:

		entry = line.split()
		percentile_coverage_list.append(float(entry[1]))

	perc_cov.close()

	percentile_coverage_total = sum(percentile_coverage_list)

	normalized_perc_cov = [i/percentile_coverage_total for i in percentile_coverage_list]

	print "length norm perc cov", len(normalized_perc_cov)
	print "percentiles", normalized_perc_cov
			...
			for event in standard_event_dict[chrom][strand]:
				...
				for form in ["included", "excluded"]:
					...
					for junction in standard_event_dict[chrom][strand][event][form + "_junction_counts"]:
						...
						if junction in standard_event_dict[chrom][strand][event]["junction_percentiles"]:

							index = standard_event_dict[chrom][strand][event]["junction_percentiles"][junction]

							if index == 100:

								index = 99

							standard_event_dict[chrom][strand][event][form + "_junction_counts"][junction] = standard_event_dict[chrom][strand][event][form + "_junction_counts"][junction]/normalized_perc_cov[index]




def max_jnc_gene_dict(gene_event_dict, standard_event_dict):
	'''
...
def main(args, event_dict = None):
	...
	parser.add_argument("--transcript_gtf", type = str, help = "Path to transcript_gtf.  Required along with --event_ioe if using end correction")
	parser.add_argument("--percentile_coverage", type = str, help = "Path to tsv file containing fraction of maximal coverage observed in transcript length percentile (where 0 begins at the 5'-end and 100 ends at the 3'-end. Required for end correction")
	parser.add_argument("--end_correction", action = "store_true", help = "Use percentile coverage information to correct junction counts based on their estimated distance from the transcript termini" )
	...
	if args.end_correction and (args.event_ioe is None or args.transcript_gtf is None or args.percentile_coverage is None):

		sys.exit("--end_correction was passed, which requires that --event_ioe, --transcript_gtf, and --percentile_coverage are all supplied with values")
	...
	if event_ioe:
		...
		gene_event_dict, event_transcript_dict = import_ioe(event_ioe)

	if args.end_correction:

		transcript_dict = splice_lib.generate_standard_transcript_dict(args.transcript_gtf)
		splice_lib.sort_transcript_dict_exons(transcript_dict)
		splice_lib.add_junctions_to_transcript_dict(transcript_dict)

		junction_percentile_dict = splice_lib.get_transcript_junction_percentiles(transcript_dict)
		get_event_junction_percentiles(standard_event_dict, event_transcript_dict, junction_percentile_dict, transcript_dict)
	...
	if args.end_correction:
		...
		end_correct_counts(standard_event_dict, args.percentile_coverage)
	...
	if calc_gene_frac:
		...
		gene_jc_dict = max_jnc_gene_dict(gene_event_dict, standard_event_dict)
